...
--------------------------------------------------------------------------------
evalE _ (EInt i)       = Right (VInt i)
evalE _ (EBool b)      = Right (VBool b)
evalE env (EVar x)       = case lookupId x env of
                           v@(VClos clEnv y e) -> Right (VClos (extend x v clEnv) y e) 
                           v -> Right v
evalE env (EBin o e1 e2) = do   { evalE env e1 
			     ; evalE env e2
			     ; Right (evalOp o (eval env e1) (eval env e2))}
evalE env (EIf c t e)    = do { evalE env c
			     ; case eval env c of
...
evalE env (EApp e1 e2)   = do { evalE env e1 
			     ; evalE env e2
			     ; Right (evalApp (eval env e1) (eval env e2))}
evalE env (ELam x e)     = Right (VClos env x e)
evalE _ ENil           = Right (VNil)

evalE env (EThr e)       = Left (eval env e)

evalE env (ETry e1 x e2) = case evalE env e1 of
				Left val -> case val of
					VInt i -> evalE env (ELet x (EInt i) e2)
					VBool j -> evalE env (ELet x (EBool j) e2) -- idk if i need this
				Right val -> Right val
